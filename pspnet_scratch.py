...
class PSPNetScratch(nn.Module):
    def __init__(self, num_classes=21, use_aux=False):
        ...
        # Grab a ResNet50. In the PSPNet paper, they use a "Dilated" ResNet where the 
        # downsampling in the last two blocks is replaced with dilated convolutions
        # to preserve spatial resolution (outputting 1/8th the size instead of 1/32nd).
        resnet = resnet50(
        ...
        # The PSPNet paper uses an Auxiliary Loss branch on the layer3 output 
        ...
        # Final classification layers to get our pixel-wise predictions
    ...
    def forward(self, x):
        ...
        if self.training and self.use_aux:
            ...
            aux_out = nn.functional.interpolate(aux_out, size=size[2:], mode='bilinear', align_corners=True)
